...
- **A real issue tracker (GitHub, Linear, …)** → publish one issue per ticket in dependency order (blockers first) so each ticket's blocking edges can reference real identifiers. 
  - For **GitHub Issues**, publish sequentially using the GitHub CLI:
    ```bash
    gh issue create --title "<Ticket title>" --body "$(cat <<'EOF'
    ## What to build
    <Description>
    ...
    ## Blocked by
    - #<BlockingIssueNumber> (or "None — can start immediately")
    EOF
    )" --label "ready-for-agent"
    ```
  - Capture the generated issue numbers (e.g. `#101`) to populate the `Blocked by` references of downstream tickets.
